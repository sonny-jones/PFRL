...
def _add_log_prob_and_value_to_episodes(
    ...
    with torch.no_grad(), pfrl.utils.evaluating(model):
        ...
        # log_probs =  log_probs_c + log_probs_d
        
        vs_pred = vs_pred.cpu().numpy().ravel()
        next_vs_pred = next_vs_pred.cpu().numpy().ravel()

    for transition, c_log_prob, d_log_prob, v_pred, next_v_pred in zip(
        dataset, c_log_probs, d_log_probs,  vs_pred, next_vs_pred
    ):
        transition["c_log_prob"] = c_log_prob
        transition["d_log_prob"] = d_log_prob
...
class HybridPPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        self.c_entropy_record = collections.deque(maxlen=entropy_stats_window)
        self.d_entropy_record = collections.deque(maxlen=entropy_stats_window)
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(
            ...
            #Hybrid_actions
            c_actions = torch.tensor([b["action"][0] for b in batch], device=device)
            d_actions = torch.tensor([b["action"][1] for b in batch], device=device)

            distribs, vs_pred = self.model(states)


            #Hybrid_actions
            c_log_probs = distribs[0].log_prob(c_actions)
            d_log_probs = distribs[1].log_prob(d_actions)
            
            total_entropy = distribs[0].entropy() + distribs[1].entropy()
            ...
            c_log_probs_old = torch.tensor(
                [b["c_log_prob"] for b in batch],
                dtype=torch.float,
                device=device,
            )
            d_log_probs_old = torch.tensor(
                [b["d_log_prob"] for b in batch],
                dtype=torch.float,
                device=device,
            )
            ...
            loss = self._lossfun(
                ...
                d_log_probs=d_log_probs,
                # distribs.log_prob(actions),
                vs_pred_old=vs_pred_old,
                #HYBRID
                c_log_probs_old=c_log_probs_old,
                d_log_probs_old=d_log_probs_old,
